...
List<Viagem> viagem = (List<Viagem>) request.getAttribute("viagens");
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<main>
		...
		<h2 class="text-center fonte-especial">Agenda de Viagem</h2>
			<div class="d-flex justify-content-end">
				<a href="./views/viagem/create.jsp" class="btn botao mb-2">
					Agendar viagem </a>
			</div>
			<table class="table table-responsive table-hover">
				...
				<tbody>
					...
					<tr>
						...
						<td><%=v.getDestino().getDestino()%></td>
						<td><%=v.getDataIda()%></td>
						<td><%=v.getDataVolta()%></td>
						<td><%=v.getTotal()%></td>
						
						<td>
							<div class="d-flex">
								<a href="./edit-viagem?id=<%=v.getId()%>" class="mx-1"
									title="Editar"> <i class="bi bi-file-earmark-text"></i>
								</a> <a href="./delet-viagem?id=<%=v.getId()%>" class="mx-1"
									title="Excluir"
									onclick="return confirm('Deseja excluir a viagem para <%=v.getDestino().getDestino()%>.')">
